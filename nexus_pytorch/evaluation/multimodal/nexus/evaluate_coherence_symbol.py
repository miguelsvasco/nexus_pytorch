...
ex = sacred.Experiment(
    'multimodal_nexus_evaluate_coherence_symbol',
    ingredients=[
...
def evaluate_recon(model, dataloader, n_samples, cuda):
    ...
    with torch.no_grad():
        ...
        for batch_idx, data in enumerate(tqdm(dataloader)):
            ...
            img_snd_acc = compute_accuracy(samples=cm_sym[1], target=labels, classifier=None)
            ...
            img_trj_acc = compute_accuracy(samples=cm_sym[1], target=labels, classifier=None)
            ...
            snd_trj_acc = compute_accuracy(samples=cm_sym[1], target=labels, classifier=None)
            double_sound_trajectory_meter.update(snd_trj_acc.item())

            # Triple mod CMI encoding
            _, _, _, cm_sym = model.generate(x_img=img_data, x_snd=snd_data, x_trj=trj_data)
            all_mods_acc = compute_accuracy(samples=cm_sym[1], target=labels, classifier=None)
...
def evaluate(model, _config, _run):
    ...
    _run.log_scalar('acc double image sound', sym_res['double_image_sound'])
    _run.log_scalar('acc double image trajectory', sym_res['double_image_trajectory'])
    _run.log_scalar('acc double sound trajectory', sym_res['double_sound_trajectory'])
    _run.log_scalar('acc all mods', sym_res['all_mods'])
