...
def save_checkpoint(state,
                    is_best,
                    folder='./',
                    filename='fashion_classifier_checkpoint.pth.tar'):
    torch.save(state, os.path.join(folder, filename))
    if is_best:
        shutil.copyfile(
            os.path.join(folder, filename),
            os.path.join(folder, 'best_fashion_classifier_model.pth.tar'))
...
def train(_config, _run):
    ...
    for epoch in range(1, epochs + 1):
        ...
        train_loss = train_epoch(model, train_loader, optimizer, criterion, epoch, cuda=gpu_config)
        val_loss, val_accuracy = test_epoch(model, test_loader, criterion, cuda=gpu_config)
    ...
    ex.add_artifact(
        os.path.join(
            log_dir_path('trained_models'), 'fashion_classifier_checkpoint.pth.tar'),
        name='fashion_classifier_last_checkpoint.pth.tar')
    ex.add_artifact(
        os.path.join(
            log_dir_path('trained_models'), 'best_fashion_classifier_model.pth.tar'),
        name='best_fashion_classifier_model.pth.tar')
