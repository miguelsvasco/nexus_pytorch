...
import shutil
import sys
import nexus_pytorch.evaluation.classifiers.mnist.ingredients as ingredients
...
def save_checkpoint(state,
                    is_best,
                    folder='./',
                    filename='mnist_classifier_checkpoint.pth.tar'):
    torch.save(state, os.path.join(folder, filename))
    if is_best:
        shutil.copyfile(
            os.path.join(folder, filename),
            os.path.join(folder, 'best_mnist_classifier_model.pth.tar'))
...
def train(_config, _run):
    ...
    ex.add_artifact(
        os.path.join(
            log_dir_path('trained_models'), 'mnist_classifier_checkpoint.pth.tar'),
        name='mnist_classifier_last_checkpoint.pth.tar')
    ex.add_artifact(
        os.path.join(
            log_dir_path('trained_models'), 'best_mnist_classifier_model.pth.tar'),
