...
def save_checkpoint(state,
                    is_best,
                    folder='./',
                    filename='trajectory_classifier_checkpoint.pth.tar'):
    torch.save(state, os.path.join(folder, filename))
    if is_best:
        shutil.copyfile(
            os.path.join(folder, filename),
            os.path.join(folder, 'best_trajectory_classifier_model.pth.tar'))
...
def train(_config, _run):
    ...
    # Create Classifier
    model = Trajectory_Classifier().to(device)
    ...
    ex.add_artifact(
        os.path.join(
            log_dir_path('trained_models'), 'trajectory_classifier_checkpoint.pth.tar'),
        name='trajectory_classifier_last_checkpoint.pth.tar')
    ex.add_artifact(
        os.path.join(
            log_dir_path('trained_models'), 'best_trajectory_classifier_model.pth.tar'),
        name='best_trajectory_classifier_model.pth.tar')
