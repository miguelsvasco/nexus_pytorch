...
import shutil
import sys
import nexus_pytorch.evaluation.autoencoders.fashion.ingredients as ingredients
import torchvision
from nexus_pytorch.scenarios.standard_dataset.standard_dataset import StandardDataset

ex = sacred.Experiment(
    'fashion_autoencoder_all',
...
def save_checkpoint(state,
    ...
    filename = 'fashion_ae_checkpoint.pth.tar'
    torch.save(state, os.path.join(folder, filename))
    if is_best:
        shutil.copyfile(
            os.path.join(folder, filename),
            os.path.join(folder, 'best_fashion_ae_model.pth.tar'))
...
def train(_config, _run):
    ...
    # Create Classifier
    model = Fashion_AE(b_dim=training_config['b_dim']).to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        train_loss = train_epoch(model, train_loader, optimizer, epoch, cuda=True)
        ...
        if epoch % artifact_storage_interval == 0:
            ...
            with torch.no_grad():
                ...
                torchvision.utils.save_image(torchvision.utils.make_grid(image_comp,
                                                                         ...
                                                                         pad_value=.5,
                                                                         nrow=data.size(0)),
                                             os.path.join(results_dir, 'fashion_ae_mod_e' + str(epoch) + '.png'))
                ex.add_artifact(os.path.join(results_dir, "fashion_ae_mod_e" + str(epoch) + '.png'),
                                name="fashion_ae_recon_e" + str(epoch) + '.png')
    ...
    ex.add_artifact(
        os.path.join(
            log_dir_path('trained_models'), 'fashion_ae_checkpoint.pth.tar'),
        name='fashion_ae_last_checkpoint.pth.tar')
    ex.add_artifact(
        os.path.join(
            log_dir_path('trained_models'), 'best_fashion_ae_model.pth.tar'),
        name='best_fashion_ae_model.pth.tar')
