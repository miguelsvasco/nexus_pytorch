...
def save_checkpoint(state,
    ...
    filename = 'mnist_ae_checkpoint.pth.tar'
    torch.save(state, os.path.join(folder, filename))
    if is_best:
        shutil.copyfile(
            os.path.join(folder, filename),
            os.path.join(folder, 'best_mnist_ae_model.pth.tar'))
...
def train(_config, _run):
        ...
        if epoch % artifact_storage_interval == 0:
            ...
            with torch.no_grad():
                ...
                torchvision.utils.save_image(torchvision.utils.make_grid(image_comp,
                                                                         ...
                                                                         pad_value=.5,
                                                                         nrow=data.size(0)),
                                             os.path.join(results_dir, 'mnist_ae_mod_e' + str(epoch) + '.png'))
                ex.add_artifact(os.path.join(results_dir, "mnist_ae_mod_e" + str(epoch) + '.png'),
    ...
    ex.add_artifact(
        os.path.join(
            log_dir_path('trained_models'), 'mnist_ae_checkpoint.pth.tar'),
        name='mnist_ae_last_checkpoint.pth.tar')
    ex.add_artifact(
        os.path.join(
            log_dir_path('trained_models'), 'best_mnist_ae_model.pth.tar'),
