...
import nexus_pytorch.evaluation.standard.fashion.mmvae.model.training_utils as utils

ex = sacred.Experiment(
    'fashion_mmvae_evaluate_accuracy',
    ingredients=[
...
def evaluate_recon(model, img_class, dataloader, n_samples, cuda):
    ...
    with torch.no_grad():
        ...
        for batch_idx, modality_data in enumerate(tqdm(dataloader)):
            ...
            img_mod_out, sym_mod_out = model.generate([img_data, sym_data])
            img_out = img_mod_out[0]
            sym_out = sym_mod_out[0]
            img_nx_sym_out = sym_mod_out[1]
            sym_nx_img_out = img_mod_out[1]
            ...
            # Symbol to image
            sym_img_acc = compute_accuracy(samples=sym_nx_img_out, target=sym_data_label, classifier=img_class)
            cm_sym_img_recon_meter.update(sym_img_acc.item())


        # Compile Results
        img_res = {'recon': img_recon_meter.avg, 'cm': cm_sym_img_recon_meter.avg}
        sym_res = {'recon': sym_recon_meter.avg, 'cm': cm_img_sym_recon_meter.avg}
...
def evaluate(model, img_class, _config, _run):
    ...
    _run.log_scalar('Image Accuracy', img_res['recon'])
    _run.log_scalar('Symbol to Image Accuracy', img_res['cm'])
    _run.log_scalar('Symbol Accuracy', sym_res['recon'])
    _run.log_scalar('Image to Symbol Accuracy', sym_res['cm'])
