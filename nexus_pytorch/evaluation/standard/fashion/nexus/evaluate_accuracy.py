...
import nexus_pytorch.evaluation.standard.fashion.nexus.ingredients as ingredients
from nexus_pytorch.scenarios.standard_dataset.standard_dataset import StandardDataset
import nexus_pytorch.evaluation.standard.fashion.nexus.model.training_utils as utils

ex = sacred.Experiment(
    'fashion_nexus_evaluate_accuracy',
    ingredients=[
...
def load_classifier(config, cuda):
    if cuda:
        checkpoint = torch.load('../../../classifiers/image/trained_models/best_fashion_classifier_model.pth.tar')
    else:
        checkpoint = torch.load(
            '../../../classifiers/image/trained_models/best_fashion_classifier_model.pth.tar',
...
def evaluate_recon(model, img_class, dataloader, n_samples, cuda):
    ...
    with torch.no_grad():
        ...
        print('\nFinal Results:\nImage Recon: ' + str(img_res['recon']) + '\t'
              + 'Image All Nexus Recon: ' + str(img_res['all_nexus']) + '\t'
              + 'Image Nexus Recon: ' + str(img_res['nexus']) + '\t'
              + '---> Symbol to Image Recon: ' + str(img_res['cm']) + '\n'
              + 'Symbol Recon: ' + str(sym_res['recon']) + '\t'
              + 'Symbol All Nexus Recon: ' + str(sym_res['all_nexus']) + '\t'
              + 'Symbol Nexus Recon: ' + str(sym_res['nexus']) + '\t'
