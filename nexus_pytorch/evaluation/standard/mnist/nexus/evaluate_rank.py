...
import nexus_pytorch.evaluation.standard.mnist.nexus.model.evaluation_utils as e_utils
from nexus_pytorch.evaluation.autoencoders.mnist.train import MNIST_AE

ex = sacred.Experiment(
    'mnist_nexus_evaluate_rank',
    ingredients=[
...
def get_ae(cuda):
    ...
    if cuda:
        checkpoint = torch.load('../../../autoencoders/mnist/trained_models/best_mnist_ae_model.pth.tar')
    else:
        checkpoint = torch.load(
            '../../../autoencoders/mnist/trained_models/best_mnist_ae_model.pth.tar',
            map_location=lambda storage, location: storage)

    ae = MNIST_AE(b_dim=128)
