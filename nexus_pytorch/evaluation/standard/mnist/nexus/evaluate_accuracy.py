...
from nexus_pytorch.evaluation.classifiers.mnist.train import MNISTClassifier
import nexus_pytorch.evaluation.standard.mnist.nexus.ingredients as ingredients
from nexus_pytorch.scenarios.standard_dataset.standard_dataset import StandardDataset
import nexus_pytorch.evaluation.standard.mnist.nexus.model.training_utils as utils

ex = sacred.Experiment(
    'mnist_nexus_evaluate_accuracy',
    ingredients=[
...
def load_classifier(config, cuda):
    if cuda:
        checkpoint = torch.load('../../../classifiers/image/trained_models/best_mnist_classifier_model.pth.tar')
    else:
        checkpoint = torch.load(
            '../../../classifiers/image/trained_models/best_mnist_classifier_model.pth.tar',
...
def evaluate_recon(model, img_class, dataloader, n_samples, cuda):
    ...
    with torch.no_grad():
        ...
        for batch_idx, modality_data in enumerate(tqdm(dataloader)):
            ...
            sym_data_label = modality_data[1]
            ...
            img_a_out, sym_a_out = model.generate(x_v=img_data, x_s=sym_data)
            img_out = img_a_out[0]
            sym_out = sym_a_out[0]
            img_nx_out = img_a_out[1]
            sym_nx_out = sym_a_out[1]
            ...
            img_all_nx_acc = compute_accuracy(samples=img_nx_out, target=sym_data_label, classifier=img_class)
            sym_all_nx_acc = compute_accuracy(samples=sym_nx_out, target=sym_data_label, classifier=None)
            img_recon_meter.update(img_acc.item())
            sym_recon_meter.update(sym_acc.item())
            img_all_nx_recon_meter.update(img_all_nx_acc.item())
            sym_all_nx_recon_meter.update(sym_all_nx_acc.item())

            # Cross modality reconstruction
            ## Image to Symbol
            img_nx_img_out, img_nx_sym_out = model.generate(x_v=img_data)
            img_nx_acc = compute_accuracy(samples=img_nx_img_out[1], target=sym_data_label, classifier=img_class)
            img_sym_acc = compute_accuracy(samples=img_nx_sym_out[1], target=sym_data_label, classifier=None)
            ...
            sym_img_out = sym_img_out[1]
            sym_img_acc = compute_accuracy(samples=sym_img_out, target=sym_data_label, classifier=img_class)
            sym_nx_acc = compute_accuracy(samples=sym_nx_sym_out[1], target=sym_data_label, classifier=None)
            sym_nx_recon_meter.update(sym_nx_acc.item())
            cm_sym_img_recon_meter.update(sym_img_acc.item())

        # Compile Results
        img_res = {'recon': img_recon_meter.avg, 'all_nexus': img_all_nx_recon_meter.avg, 'nexus': img_nx_recon_meter.avg, 'cm': cm_sym_img_recon_meter.avg}
        sym_res = {'recon': sym_recon_meter.avg, 'all_nexus': sym_all_nx_recon_meter.avg, 'nexus': sym_nx_recon_meter.avg, 'cm': cm_img_sym_recon_meter.avg}

        print('\nFinal Results:\nImage Accuracy: ' + str(img_res['recon']) + '\t'
              + 'Image All Nexus Accuracy: ' + str(img_res['all_nexus']) + '\t'
              + 'Image Nexus Accuracy: ' + str(img_res['nexus']) + '\t'
              + '---> Symbol to Image Accuracy: ' + str(img_res['cm']) + '\n'
              + 'Symbol Accuracy: ' + str(sym_res['recon']) + '\t'
              + 'Symbol All Nexus Accuracy: ' + str(sym_res['all_nexus']) + '\t'
              + 'Symbol Nexus Accuracy: ' + str(sym_res['nexus']) + '\t'
              + '---> Image to Symbol Accuracy: ' + str(sym_res['cm']) + '\n')
...
def evaluate(model, img_class, _config, _run):
    ...
    # Save
    with open(os.path.join(exp_dir_path('evaluation'), "recon_res.txt"), 'w') as f:
        print('\nFinal Results:\nImage Accuracy: ' + str(img_res['recon']) + '\t'
              + 'Image All Nexus Accuracy: ' + str(img_res['all_nexus']) + '\t'
              + 'Image Nexus Accuracy: ' + str(img_res['nexus']) + '\t'
              + '---> Symbol to Image Accuracy: ' + str(img_res['cm']) + '\n'
