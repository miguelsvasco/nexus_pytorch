...
ex = sacred.Experiment(
    'dropout_nexus_evaluate_accuracy',
    ingredients=[
...
def evaluate_recon(model, img_class, dataloader, n_samples, cuda):
    ...
    with torch.no_grad():
        ...
        print('\n Accuracy results:')
        print(" - All modality Accuracy = " + str((img_res['all_nexus'] + sym_res['all_nexus'])/2.))
        print(" - Single modality Accuracy = " + str((img_res['nexus'] + sym_res['nexus']) / 2.))
        print(" - Cross modality Accuracy = " + str((img_res['cm'] + sym_res['cm']) / 2.))
        print("\n")
