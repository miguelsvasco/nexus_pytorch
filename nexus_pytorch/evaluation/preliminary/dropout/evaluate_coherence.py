...
ex = sacred.Experiment(
    'dropout_nexus_evaluate_coherence',
    ingredients=[
...
def evaluate_recon(model, img_class, dataloader, n_samples, cuda):
    ...
    with torch.no_grad():
        ...
        print('\n Coherence results:')
        print(" - All modality Coherence = " + str((img_res['all_nexus'] + sym_res['all_nexus'])/2.))
        print(" - Single modality Coherence = " + str((img_res['nexus'] + sym_res['nexus']) / 2.))
        print(" - Cross modality Coherence = " + str((img_res['cm'] + sym_res['cm']) / 2.))
        print("\n")
...
def evaluate(model, img_class, _config, _run):
    ...
    # Save
    with open(os.path.join(exp_dir_path('evaluation'), "img_coherence_res.pt"), 'wb') as f:
        torch.save(img_res, f)
    ex.add_artifact(os.path.join(exp_dir_path('evaluation'),
                                 "img_coherence_res.pt"), name='img_coherence_res.pt')

    with open(os.path.join(exp_dir_path('evaluation'), "sym_coherence_res.pt"), 'wb') as f:
        torch.save(sym_res, f)
    ex.add_artifact(os.path.join(exp_dir_path('evaluation'),
                                 "sym_coherence_res.pt"), name='sym_coherence_res.pt')
    return
