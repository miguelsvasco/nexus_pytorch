...
from nexus_pytorch.evaluation.preliminary.hierarchy.s_nexus.model.trainer import Trainer
...
def post_cb(info):
    ex.add_artifact(
        os.path.join(
            log_dir_path('trained_models'), 's_nexus_checkpoint.pth.tar'),
        name='s_nexus_last_checkpoint.pth.tar')
    ex.add_artifact(
        os.path.join(
            log_dir_path('trained_models'), 'best_s_nexus_model.pth.tar'),
        name='best_s_nexus_model.pth.tar')
