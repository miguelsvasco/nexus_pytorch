from nexus_pytorch.evaluation.preliminary.hierarchy.h_nexus.model.model_components import *
...
class NexusModel(nn.Module):
    ...
    def generate(self, x_v=None, x_s=None):
        with torch.no_grad():
            ...
            if x_v is not None:
                img_out, img_z, _,  _ = self.img_mod_vae(x_v)
            else:
                img_out, img_z = None, None

            # Encode Symbolic Modality Data
            if x_s is not None:
                sym_out, sym_z, sym_mu, _ = self.sym_mod_vae(x_s)
            else:
