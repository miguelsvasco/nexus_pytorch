...
from nexus_pytorch.evaluation.preliminary.hierarchy.h_nexus.model.trainer import Trainer
from nexus_pytorch.evaluation.preliminary.hierarchy.h_nexus.model.model import NexusModel
import nexus_pytorch.evaluation.preliminary.hierarchy.h_nexus.model.training_utils as t_utils
from nexus_pytorch.scenarios.multimodal_dataset.multimodal_dataset import MultimodalDataset

ex = sacred.Experiment(
    'hierarchy_h_nexus_train',
    ingredients=[
...
class PostEpochCb(object):
    ...
    def _record_artifacts(self, info, _config):
        ...
        if epoch % artifact_storage_interval == 0:
            ...
            torchvision.utils.save_image(torchvision.utils.make_grid(all_nx_img_comp,
                                                                     padding=5,
                                                                     pad_value=.5,
                                                                     nrow=img_data.size(0)),
                                         os.path.join(results_dir, 'all_nx_img_comp_e' + str(epoch) + '.png'))
...
def post_cb(info):
    ex.add_artifact(
        os.path.join(
            log_dir_path('trained_models'), 'h_nexus_checkpoint.pth.tar'),
        name='h_nexus_last_checkpoint.pth.tar')
    ex.add_artifact(
        os.path.join(
            log_dir_path('trained_models'), 'best_h_nexus_model.pth.tar'),
        name='best_h_nexus_model.pth.tar')
