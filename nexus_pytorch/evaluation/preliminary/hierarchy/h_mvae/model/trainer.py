...
from nexus_pytorch.evaluation.preliminary.hierarchy.h_mvae.model.training_utils import AverageMeter, WarmUp
...
class Trainer(object):
    ...
    def loss_function(self, img_data=None, sym_data=None, img_out=None, sym_out=None,
                      img_dist=None, sym_dist=None, nx_mu=None, nx_logvar=None):

        # Modality Recon Terms
        img_recon, sym_recon, img_mod_prior, sym_mod_prior, img_nx_recon, sym_nx_recon = 0, 0, 0, 0, 0, 0  # default params
    ...
    def _run(self, train, epoch, dataloader, cuda):
        ...
        for batch_idx, modality_data in enumerate(dataloader):
            ...
            # Subsampling Forward
            img_out, sym_out, img_dist, sym_dist, nx_mu, nx_logvar = self.model(x_v=img_data, x_s=sym_data)
            m_m_out, m_t_out, m_m_dist, m_t_dist, m_mu, m_logvar = self.model(x_v=img_data, x_s=None)
            t_m_out, t_t_out, t_m_dist, t_t_dist, t_mu, t_logvar = self.model(x_v=None, x_s=sym_data)

            # Losses
            loss, img_recon, sym_recon, \
            img_mod_prior, sym_mod_prior, \
            img_nx_recon, sym_nx_recon, \
            nx_prior = self.loss_function(img_data=img_data, sym_data=sym_data,
                                          img_out=img_out, sym_out=sym_out,
                                          img_dist=img_dist, sym_dist=sym_dist,
                                          nx_mu=nx_mu, nx_logvar=nx_logvar)

            m_loss, _, _, _, _, _, _, _ = self.loss_function(img_data=img_data, sym_data=None, img_out=m_m_out, sym_out=None,
                                                             img_dist=m_m_dist, sym_dist=None, nx_mu=m_mu, nx_logvar=m_logvar)

            t_loss, _, _, _, _, _, _, _ = self.loss_function(img_data=None, sym_data=sym_data, img_out=None, sym_out=t_t_out,
                                                             img_dist=None, sym_dist=t_t_dist, nx_mu=t_mu, nx_logvar=t_logvar)
