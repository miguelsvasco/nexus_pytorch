...
from nexus_pytorch.evaluation.preliminary.hierarchy.h_mvae.model.model_components import *
...
class MVAE(nn.Module):
    ...
    def generate(self, x_v=None, x_s=None):
        with torch.no_grad():
            ...
            mu, logvar = self.infer(img_z, sym_z)
            ...
            return [img_out, nx_img_out], [sym_out, nx_sym_out]
    ...
    def forward(self, x_v=None, x_s=None):
        ...
        if x_s is not None:
            ...
            sym_msg = None
        mu, logvar = self.infer(img_msg, sym_msg)
        ...
        return img_out, sym_out,\
               [img_mu, img_logvar, img_z, img_nx_z],\
               [sym_mu, sym_logvar, sym_z, sym_nx_z],  mu, logvar,
