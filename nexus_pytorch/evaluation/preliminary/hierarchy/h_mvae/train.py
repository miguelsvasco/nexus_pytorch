...
from nexus_pytorch.evaluation.preliminary.hierarchy.h_mvae.model.trainer import Trainer
from nexus_pytorch.evaluation.preliminary.hierarchy.h_mvae.model.model import MVAE
import nexus_pytorch.evaluation.preliminary.hierarchy.h_mvae.model.training_utils as t_utils
from nexus_pytorch.scenarios.multimodal_dataset.multimodal_dataset import MultimodalDataset

ex = sacred.Experiment(
    'hierarchy_h_mvae_train',
    ingredients=[
...
def post_cb(info):
    ex.add_artifact(
        os.path.join(
            log_dir_path('trained_models'), 'h_mvae_checkpoint.pth.tar'),
        name='h_mvae_last_checkpoint.pth.tar')
    ex.add_artifact(
        os.path.join(
            log_dir_path('trained_models'), 'best_h_mvae_model.pth.tar'),
        name='best_h_mvae_model.pth.tar')
