...
from nexus_pytorch.evaluation.preliminary.hierarchy.s_mmvae.model.trainer import Trainer
from nexus_pytorch.evaluation.preliminary.hierarchy.s_mmvae.model.model import MMVAE
import nexus_pytorch.evaluation.preliminary.hierarchy.s_mmvae.model.training_utils as t_utils
from nexus_pytorch.scenarios.multimodal_dataset.multimodal_dataset import MultimodalDataset

ex = sacred.Experiment(
    'hierarchy_s_mmvae_train',
    ingredients=[
...
def post_cb(info):
    ex.add_artifact(
        os.path.join(
            log_dir_path('trained_models'), 's_mmvae_checkpoint.pth.tar'),
        name='s_mmvae_last_checkpoint.pth.tar')
    ex.add_artifact(
        os.path.join(
            log_dir_path('trained_models'), 'best_s_mmvae_model.pth.tar'),
        name='best_s_mmvae_model.pth.tar')
