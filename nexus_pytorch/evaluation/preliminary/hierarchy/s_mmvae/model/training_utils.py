...
def save_checkpoint(state,
                    is_best,
                    folder='./',
                    filename='s_mmvae_checkpoint.pth.tar'):
    torch.save(state, os.path.join(folder, filename))
    if is_best:
        shutil.copyfile(
            os.path.join(folder, filename),
            os.path.join(folder, 'best_s_mmvae_model.pth.tar'))
...
def load_checkpoint(checkpoint_file, use_cuda=False):
    ...
    nx_info, img_info, sym_info = get_specs(model_config)
    model = MMVAE(img_info=img_info,
                 sym_info=sym_info,
                 nx_info=nx_info,
                 use_cuda=use_cuda)
