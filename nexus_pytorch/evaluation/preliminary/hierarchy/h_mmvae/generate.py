...
def generate_class(model, label_class, n_samples, cuda):
    ...
    # Encode Mod Latents
    _, nx_out = model.generate([None, symbol])
...
def generate_prior_nexus_(model, n_samples, cuda):
    ...
    m_out = model.img_mod_vae.mod_decoder(v_q_z)
    return m_out


def generate_prior_mod(model, n_samples, cuda):
    z = Variable(torch.randn(n_samples, model.img_info['mod_latent_dim']))
    if cuda:
        z = z.cuda()

    # Decode
    m_out = model.img_mod_vae.mod_decoder(z)
    return m_out
