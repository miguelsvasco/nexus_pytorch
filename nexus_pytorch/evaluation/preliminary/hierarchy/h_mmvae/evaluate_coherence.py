...
import nexus_pytorch.evaluation.preliminary.hierarchy.h_mmvae.ingredients as ingredients
from nexus_pytorch.scenarios.multimodal_dataset.multimodal_dataset import MultimodalDataset
import nexus_pytorch.evaluation.preliminary.hierarchy.h_mmvae.model.training_utils as utils

ex = sacred.Experiment(
    'hierarchy_h_mmvae_evaluate_coherence',
    ingredients=[
...
def evaluate_recon(model, img_class, dataloader, n_samples, cuda):
    ...
    with torch.no_grad():
        ...
        for batch_idx, modality_data in enumerate(tqdm(dataloader)):
            ...
            outs, nx_outs = model.generate([img_data, sym_data])
            img_out = outs[0]
            sym_out = outs[1]
            img_nx_out     = nx_outs[0][0]
            img_nx_sym_out = nx_outs[0][1]
            sym_nx_img_out = nx_outs[1][0]
            sym_nx_out     = nx_outs[1][1]
            ...
            # Image to symbol
            img_nx_acc = compute_accuracy(samples=img_nx_out, target=sym_data_label, classifier=img_class)
            ...
            sym_nx_acc = compute_accuracy(samples=sym_nx_out, target=sym_data_label, classifier=None)
