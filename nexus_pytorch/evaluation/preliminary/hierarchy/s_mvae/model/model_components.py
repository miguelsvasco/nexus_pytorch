...
class ImageAEEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        h = x.view(x.size(0), -1)
        h = self.classifier(h)
        return self.fc_out_norm(self.fc_out(h))




# Image Decoder
class ImageAEDecoder(nn.Module):
    def __init__(self, name, input_dim, n_channels, conv_layers, linear_layers, output_dim):
        super(ImageAEDecoder, self).__init__()
...
# Symbol
class SymbolAEEncoder(nn.Module):
    def __init__(self, name, input_dim, layer_sizes, output_dim):
        super(SymbolAEEncoder, self).__init__()
        ...
        # Output layer of the network
        self.fc_out = nn.Linear(pre, output_dim)
        self.fc_norm_1 = nn.BatchNorm1d(output_dim)
        self.fc_norm_2 = nn.LeakyReLU()
    ...
    def forward(self, x):
        h = self.network(x)
        h = self.fc_norm_1(self.fc_out(h))
        return self.fc_norm_2(h)



class SymbolAEDecoder(nn.Module):
    def __init__(self, name, input_dim, layer_sizes, output_dim):
        super(SymbolAEDecoder, self).__init__()
        ...
        # Create Network
        dec_layers = []
        dec_layers.append(nn.BatchNorm1d(input_dim))
